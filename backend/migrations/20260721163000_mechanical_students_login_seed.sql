-- Mechanical Engineering student login accounts (department import CSV)
...
BEGIN
  ...
  SELECT dept_id INTO v_dept FROM departments WHERE dept_name = 'Mech Engg' LIMIT 1;
  ...
  IF v_tenant IS NULL OR v_dept IS NULL OR v_role IS NULL OR v_entity IS NULL THEN
    RAISE NOTICE 'Skipping ME student seed — tenant/dept/role/entity missing';
    RETURN;
  END IF;
...
VALUES (
  'mechanical.students-login',
  'student',
  'aryan.2455698@mygyanvihar.com',
  'Mechanical student portal logins',
  '39 ME students · password123',
  'Upserts Mech Engg student accounts from department import CSV for production pilot.'
)
