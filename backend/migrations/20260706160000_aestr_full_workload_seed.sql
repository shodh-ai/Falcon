...
-- Strip spaces and merge spaced-code orphans when canonical code already exists.
DO $$
DECLARE
  r RECORD;
  m RECORD;
...
DELETE FROM academic_course_allocations drop_a
 USING academic_courses spaced,
       academic_courses canon,
       academic_course_allocations keep_a
 WHERE drop_a.course_id = spaced.course_id
   AND spaced.course_code ~ '\s'
   AND canon.tenant_id = spaced.tenant_id
   AND canon.course_code = UPPER(REPLACE(TRIM(spaced.course_code), ' ', ''))
   AND canon.course_id <> spaced.course_id
   AND keep_a.course_id = canon.course_id
   AND keep_a.tenant_id = drop_a.tenant_id
   AND keep_a.subject_id = drop_a.subject_id
   AND keep_a.program_name IS NOT DISTINCT FROM drop_a.program_name
   AND keep_a.semester IS NOT DISTINCT FROM drop_a.semester
   AND keep_a.academic_year = drop_a.academic_year;
 ...
 WHERE a.course_id = spaced.course_id
   ...
   AND NOT EXISTS (
     ...
     WHERE x.tenant_id = a.tenant_id
       ...
       AND x.semester IS NOT DISTINCT FROM a.semester
       AND x.academic_year = a.academic_year
       AND x.course_id = canon.course_id
   );
