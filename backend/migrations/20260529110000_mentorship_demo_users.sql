...
INSERT INTO users (user_id, name, official_email, role_id, is_active)
VALUES (
...
DO UPDATE SET role_id = (SELECT role_id FROM roles WHERE role_name = 'Faculty' LIMIT 1);
...
ON CONFLICT (official_email) 
DO UPDATE SET role_id = (SELECT role_id FROM roles WHERE role_name = 'Student' LIMIT 1);
