...
export class MoonshotsController {
  ...
  projects(@Req() req: { user: AuthUser }) {
    ...
    const isStudentOnly =
      roles.map((r) => r.toLowerCase()).includes('student') &&
      !roles.some((r) =>
        ['faculty', 'labadmin', 'wrangler', 'iqac', 'dean', 'superadmin', 'campusadmin'].includes(
          r.toLowerCase(),
        ),
      );
    return this.moonshots.listProjects(
      this.tenant(req),
      isStudentOnly ? req.user.user_id : undefined,
    );
  }
  ...
  mine(@Req() req: { user: AuthUser }) {
    return this.moonshots.listProjects(this.tenant(req), req.user.user_id);
  }
  ...
  create(
    ...
    @Body() body: { program_id: string; title: string; disclosure_notes?: string },
  ) {
    return this.moonshots.createProject(this.tenant(req), req.user.user_id, body);
  }
