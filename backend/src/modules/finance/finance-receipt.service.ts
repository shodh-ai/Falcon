...
export class FinanceReceiptService {
  ...
  async generateAndStore(params: {
    ...
    const student = students[0] as { name: string; official_email: string; enrollment_no: string } | undefined;
    ...
    page.drawText('Suresh Gyan Vihar University', { x: 50, y: 780, size: 16, font: bold, color: navy });
    page.drawText('Fee Payment Receipt', { x: 50, y: 755, size: 12, font });
    page.drawText(`Receipt No: ${params.receiptNumber}`, { x: 50, y: 720, size: 10, font });
    page.drawText(`Transaction ID: ${params.transactionId}`, { x: 50, y: 700, size: 10, font });
    page.drawText(`Date: ${new Date().toLocaleString('en-IN')}`, { x: 50, y: 680, size: 10, font });
    page.drawText(`Student: ${student?.name ?? params.studentUserId}`, { x: 50, y: 650, size: 10, font });
    page.drawText(`Enrollment: ${student?.enrollment_no ?? '—'}`, { x: 50, y: 630, size: 10, font });
    page.drawText(`Fee Head: ${params.feeHead ?? 'University Fees'}`, { x: 50, y: 610, size: 10, font });
    page.drawText(`Amount Paid: ₹${params.amount.toFixed(2)}`, { x: 50, y: 580, size: 12, font: bold, color: navy });
    page.drawText(`Mode: ${params.paymentMode ?? 'Online'}`, { x: 50, y: 560, size: 10, font });
    page.drawText('GSTIN: 08AAECS1234F1Z5 (University)', { x: 50, y: 520, size: 9, font });
    page.drawText('This is a computer-generated receipt.', { x: 50, y: 100, size: 8, font });

    const bytes = await pdf.save();
    const relDir = path.join(params.tenantId, 'receipts', new Date().getFullYear().toString());
  ...
  async emailReceipt(studentUserId: string, receiptUrl: string, amount: number) {
    const rows = await this.dataSource.query(
      `SELECT u.name, u.official_email FROM users u WHERE u.user_id = $1`,
      [studentUserId],
    );
    const student = rows[0] as { name: string; official_email: string } | undefined;
    ...
    const bcc = (parents as Array<{ official_email: string }>).map((p) => p.official_email).filter(Boolean);
    ...
    try {
      await this.transporter.sendMail({
        from: this.config.get('EMAIL_FROM', 'Falcon Finance <noreply@falcon.local>'),
      ...
      this.logger.warn(`Receipt email failed: ${err instanceof Error ? err.message : err}`);
    }
  }
}
