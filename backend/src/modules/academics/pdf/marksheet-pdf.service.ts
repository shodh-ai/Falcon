...
export class MarksheetPdfService {
  ...
  async generate(
    ...
    const gradeCard = await this.loadGradeCard(studentUserId, tenantId, semester);
    if (gradeCard?.status === 'WITHHELD') {
      throw new ForbiddenException(String(gradeCard.payload?.withheld_reason ?? 'Marksheet is withheld'));
    }
    if (type === 'final' && gradeCard?.payload?.result_stage !== 'FINAL') {
      throw new ForbiddenException('Final marksheet is not published yet');
    }
    if (type === 'provisional' && gradeCard && gradeCard.payload?.result_stage === 'DRAFT') {
      throw new ForbiddenException('Provisional marksheet is not published yet');
    }
    ...
    const sgpa = payload?.sgpa != null ? Number(payload.sgpa).toFixed(2) : this.calculateSgpa(rows);
    ...
    page.drawText(isFinal ? 'Falcon Campus OS - Final' : 'Generated via Falcon Campus OS - Provisional', {
      x: isFinal ? 160 : 120,
      y: height / 2,
      size: isFinal ? 34 : 36,
      font: bold,
    ...
    y -= 22;
    page.drawText(isFinal ? 'Final Marksheet' : 'Provisional Marksheet', { x: 50, y, size: 18, font: bold });
    y -= 28;
    ...
    if (rank) {
      page.drawText(`Semester Rank: ${rank}`, { x: 360, y, size: 12, font: bold });
    }

    y -= 18;
    page.drawText('Formula: SGPA/CGPA = sum(credits x grade points) / sum(attempted credits)', {
      x: 50,
      y,
      size: 8,
      font,
    ...
    const verifyId = String(payload?.verification_id ?? `FALCON-${studentUserId.slice(0, 8)}-S${semester}`);
    page.drawText(`Digital verification ID: ${verifyId}`, { x: 50, y: 70, size: 9, font });
    page.drawText(
      isFinal ? 'Digitally issued by Controller of Examinations' : 'Subject to final verification by Controller of Examinations',
  ...
  private async loadGradeCard(studentUserId: string, tenantId: string, semester: number) {
