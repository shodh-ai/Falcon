...
export const STUDENT_ONBOARDING_DOC_TYPES = [
  'AADHAAR',
  '10TH_MARKSHEET',
  '12TH_MARKSHEET',
  'PHOTO',
] as const;
export const STAFF_ONBOARDING_DOC_TYPES = [
  'AADHAAR',
  'PAN',
  'HIGHEST_DEGREE',
  'PHOTO',
...
export type StudentOnboardingDocType =
  (typeof STUDENT_ONBOARDING_DOC_TYPES)[number];
export type StaffOnboardingDocType =
  (typeof STAFF_ONBOARDING_DOC_TYPES)[number];

const STAFF_ROLES = new Set(['faculty', 'hod', 'dean']);
const FIRST_LOGIN_WIZARD_ROLES = new Set([
  'student',
  'applicant',
  'faculty',
  'hod',
...
export function requiresFirstLoginWizard(
...
export function getInitialOnboardingStatusForRole(
  roleName: string | undefined | null,
): string {
  return requiresFirstLoginWizard(roleName)
...
export function resolveOnboardingPortalKind(
  roleName: string | undefined | null,
): OnboardingPortalKind {
...
export function getDashboardPathForRoleName(
  roleName: string | undefined | null,
): string {
  const role = (roleName ?? '').trim().toLowerCase();
