...
export async function fetchDepartmentHodUserId(
  query: (sql: string, params?: unknown[]) => Promise<Array<{ hod_user_id: string | null }>>,
  deptId: number | null | undefined,
): Promise<string | null> {
  if (!deptId) return null;
  const rows = await query(`SELECT hod_user_id FROM departments WHERE dept_id = $1 LIMIT 1`, [deptId]);
  return rows[0]?.hod_user_id ?? null;
}

const MANAGER_PORTAL_ROLES = new Set(['hod', 'dean', 'hr', 'hradmin', 'superadmin', 'president']);
