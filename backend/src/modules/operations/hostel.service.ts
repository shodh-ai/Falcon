...
export class HostelService {
  constructor(
    ...
    private allocations: Repository<HostelAllocation>,
    @InjectRepository(HostelRequest)
    private requests: Repository<HostelRequest>,
  ...
  async getMyAllocation(studentUserId: string) {
    ...
    const [room, warden] = await Promise.all([
      ...
      allocation.warden_user_id
        ? this.users.findOne({ where: { user_id: allocation.warden_user_id } })
  ...
  async createRequest(studentUserId: string, dto: CreateHostelRequestDto) {
    ...
    const warden =
      await this.workflowRouting.getWardenForStudent(studentUserId);
    ...
    if (isGatePass) {
      const room = await this.allocations.manager.query<
        Array<{ hostel_id: string }>
      >(
      ...
      if (room[0]?.hostel_id) {
        this.hostelDeskGateway.emitToHostel(
          room[0].hostel_id,
          'gate_pass.updated',
          {
            request_id: saved.request_id,
  ...
  async createLeave(
    ...
    const hostelRows = await this.allocations.manager.query<
      Array<{ hostel_id: string }>
