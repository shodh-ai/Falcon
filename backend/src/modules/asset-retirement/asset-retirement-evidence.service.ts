...
@Injectable()
export class AssetRetirementEvidenceService {
  constructor(
  ...
  private validate(file: Express.Multer.File) {
    ...
    const mime = file.mimetype.toLowerCase(),
      ...
      bytes = file.buffer;
    if (!ALLOWED.has(mime))
      throw new BadRequestException('Unsupported retirement evidence type');
  ...
  private async scan(buffer: Buffer, extension: string) {
    const command =
        process.env.ASSET_RETIREMENT_CLAMAV_COMMAND?.trim() ||
        process.env.ASSET_SERVICE_CLAMAV_COMMAND?.trim(),
      required =
        process.env.ASSET_RETIREMENT_REQUIRE_MALWARE_SCAN === 'true' ||
    ...
    const directory = await mkdtemp(join(tmpdir(), 'falcon-retirement-')),
      target = join(directory, `evidence${extension}`);
    try {
  ...
  async upload(
    ...
    const contentHash = retirementHash(file.buffer),
      objectKey = this.storage.buildKey(
        actor.tenant_id,
        `asset-retirement/${caseId}/${contentHash}${extension}`,
      );
