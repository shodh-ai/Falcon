...
export class ExecutiveActionService {
  ...
  private execNotify(title: string, message: string, actionLink?: string): NotificationMessage {
    return {
      category: 'OPERATIONS',
      title,
      message,
    ...
    await this.db.query(
      `INSERT INTO executive_approval_thresholds
       ...
       ON CONFLICT (tenant_id, category) DO UPDATE SET
         ...
         updated_at = NOW()`,
      [tid, dto.category, dto.auto_approve_below, dto.chairman_approval_above, userId],
    );
    return { ok: true };
  }
  ...
  async listTasks(tenantId?: string) {
    ...
    return rows.map((r: Record<string, unknown>) => ({
      ...r,
      overdue: ['OPEN', 'IN_PROGRESS'].includes(String(r.status)) && new Date(String(r.due_at)) < new Date(),
    }));
  }
  ...
  async createTask(
    ...
    await this.notifyDispatch.dispatch({
      ...
      ...this.execNotify('Executive task assigned', dto.title, '/leadership/tasks'),
      queueDelivery: true,
    }).catch(() => undefined);
    return task;
  }
  ...
  async sendMemo(
    ...
    const memoRows = await this.db.query(
      ...
      [tid, dto.subject, dto.body, dto.audience_roles, senderId, dto.confidential ?? true],
    );
  ...
  async sendBroadcast(
    ...
    const rows = await this.db.query(
      ...
      [tid, dto.subject, dto.body, dto.channels, dto.audience_filter, senderId, users.length],
    );
    return rows[0];
  }
  ...
  async registerDocument(
    ...
    const rows = await this.db.query(
      ...
      [tid, dto.title, dto.category, dto.storage_key, dto.expires_at ?? null, userId],
    );
  ...
  async getPredictiveForecast(tenantId?: string) {
    ...
    const years = yoy as Array<{ year: number; inquiries: number; enrolled: number }>;
    const avgConversion =
      years.length > 0
        ? years.reduce((s, y) => s + (y.inquiries ? y.enrolled / y.inquiries : 0), 0) / years.length
        : 0.05;

    const programForecasts = (programs as Array<Record<string, unknown>>).map((p) => {
      const cap = Number(p.capacity ?? 0);
      const en = Number(p.enrolled ?? 0);
      const seatsOpen = Math.max(cap - en, 0);
      const leadsNeeded = avgConversion > 0 ? Math.ceil(seatsOpen / avgConversion) : seatsOpen * 20;
      return {
    ...
    const bedsAvailable = Number(hostelRow.total ?? 0) - Number(hostelRow.occupied ?? 0);
  ...
  async getGrievanceEscalationMatrix(tenantId?: string) {
    ...
    return {
      ...
      alerts: (rows as Array<{ category: string; open_count: number }>).filter(
        (r) => sensitive.includes(String(r.category).toUpperCase()) && r.open_count > 0,
      ),
    };
  }
}
