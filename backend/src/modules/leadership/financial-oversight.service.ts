...
export class FinancialOversightService {
  ...
  async getMacroBudget(tenantId?: string, financialYear?: string) {
    ...
    return {
      ...
      departments: depts.map((r: Record<string, unknown>) => ({
        ...
        consumed:
          Number(r.utilized_amount ?? 0) + Number(r.encumbered_amount ?? 0),
    ...
    return this.db.transaction(async (tx) => {
      ...
      if (amount > available) {
        throw new BadRequestException(
          `Only ₹${available.toLocaleString()} available to move`,
        );
      }
  ...
  async checkPoBudgetLimit(
    ...
    const remaining =
      ...
      Number(row.utilized_amount) -
      Number(row.encumbered_amount);
  ...
  async getTreasuryView(tenantId?: string) {
    ...
    return {
      ...
      bank_accounts: banks,
      upcoming_payroll_estimate:
        Number(payroll[0]?.next_month_estimate ?? 0) / 3,
      outstanding_receivables: Number(outstanding[0]?.total ?? 0),
      net_liquidity:
        cashPosition - Number(payroll[0]?.next_month_estimate ?? 0) / 3,
    };
  }

  async getExpenseOversight(tenantId?: string) {
    const tid = this.tenantId(tenantId);
    const [thresholds, payroll, vendors, costPerStudent, poQueue] =
      await Promise.all([
        this.db
          .query(
    ...
    return {
      ...
      total_debt: loans.reduce(
        (s: number, l: Record<string, unknown>) =>
          s + Number(l.principal_remaining ?? 0),
        0,
      ),
