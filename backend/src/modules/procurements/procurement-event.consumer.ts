...
export class ProcurementEventConsumer {
  ...
  @Interval(10_000)
  async consumePhysicalVerificationCompletion() {
    const rows = await this.db.query(
      `SELECT e.event_id FROM pv_outbox_events e
       JOIN tenant_subscriptions ts ON ts.tenant_id=e.tenant_id
         AND ts.feature_key='dofa_module4_product_verification' AND ts.is_enabled=true
       WHERE e.event_type='PhysicalVerificationLineCompleted.v1'
         AND NOT EXISTS (SELECT 1 FROM proc_physical_verification_event_consumption c WHERE c.event_id=e.event_id)
       ORDER BY e.created_at LIMIT 20`,
    );
    for (const row of rows as Array<{ event_id: string }>) {
      try {
        await this.procurements.applyPhysicalVerificationCompletion(
          row.event_id,
        );
      } catch (error) {
        this.logger.error(
          `Failed to apply physical verification completion ${row.event_id}`,
          error instanceof Error ? error.stack : String(error),
        );
      }
    }
  ...
  @Interval(10_000)
  async consumeInventoryCompletion() {
    const rows = await this.db.query(
      `SELECT e.event_id FROM inv_outbox_events e
       JOIN tenant_subscriptions ts ON ts.tenant_id=e.tenant_id
         AND ts.feature_key='dofa_module5_inventory' AND ts.is_enabled=true
       WHERE e.event_type='InventoryLineCompleted.v1'
         AND NOT EXISTS (SELECT 1 FROM proc_inventory_event_consumption c WHERE c.event_id=e.event_id)
       ORDER BY e.created_at LIMIT 20`,
    );
    for (const row of rows as Array<{ event_id: string }>) {
      try {
        await this.procurements.applyInventoryLineCompletion(row.event_id);
      } catch (error) {
        this.logger.error(
          `Failed to apply inventory completion ${row.event_id}`,
          error instanceof Error ? error.stack : String(error),
        );
      }
    }
