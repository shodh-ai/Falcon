  ...
  async createUser(tenantId: string, actorId: string, dto: CreateAdminUserDto) {
    ...
    try {
      await this.db.query(
        ...
        [row.user_id, roleId, tid],
      );
    } catch {
      // user_roles schema varies across tenants
    }
  ...
  async createAnnouncement(
    ...
    const [row] = await this.db.query(
      `INSERT INTO admin_announcements
         (tenant_id, title, body, category, audience, is_emergency, created_by, updated_by)
