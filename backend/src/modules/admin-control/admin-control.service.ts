...
export class AdminControlService {
  ...
  async listAuditLogs(
    ...
    if (opts.resource_type?.trim()) {
      params.push(opts.resource_type.trim().toLowerCase());
      where.push(`lower(a.resource_type) = $${params.length}`);
    }
    if (opts.q?.trim()) {
      params.push(`%${opts.q.trim().toLowerCase()}%`);
      where.push(
        `(lower(a.resource_type) LIKE $${params.length}
          OR lower(COALESCE(u.name,'')) LIKE $${params.length}
          OR lower(COALESCE(u.email,'')) LIKE $${params.length}
          OR lower(COALESCE(a.resource_id,'')) LIKE $${params.length}
          OR lower(a.action) LIKE $${params.length})`,
      );
    }
    ...
    try {
      ...
      const [statsRow] = await this.db.query(
        `SELECT
           COUNT(*) FILTER (WHERE created_at >= date_trunc('day', NOW()))::int AS today,
           COUNT(*) FILTER (WHERE action = 'LOGIN' AND created_at >= date_trunc('day', NOW()))::int AS logins_today
         FROM admin_control_audit
         WHERE tenant_id = $1`,
        [tid],
      );
      const items = await this.db.query(
        `SELECT a.audit_id, a.action, a.resource_type, a.resource_id, a.details,
                a.created_at, a.actor_user_id, a.ip_address,
      ...
      return {
        ...
        stats: {
          ...
          logins_today: Number(statsRow?.logins_today ?? 0),
        },
      };
    } catch {
      const fallback = await this.db.query(
        `SELECT log_id AS audit_id, action, table_name AS resource_type,
                ...
                NULL AS actor_email, NULL AS ip_address
