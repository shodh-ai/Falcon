...
export class EntityCreatorGuard implements CanActivate {
  ...
  canActivate(context: ExecutionContext): boolean {
    const required = this.reflector.getAllAndOverride<boolean>(ENTITY_CREATOR_KEY, [
      context.getHandler(),
      context.getClass(),
    ]);
    ...
    if (email !== ENTITY_CREATOR_EMAIL) {
      throw new ForbiddenException('Entity management is restricted to the master Super Admin account');
    }
    return true;
  }
}
