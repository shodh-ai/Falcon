...
export class HandoverController {
  ...
  @Post()
  @Roles('IQAC', 'HR')
  performHandover(@Body() createHandoverDto: CreateHandoverDto, @Req() req: any) {
    return this.handoverService.performHandover(createHandoverDto, req.user.user_id);
  }

  @Get('history')
  @Roles('IQAC', 'HR', 'President', 'Dean')
  getHandoverHistory(@Req() req: any) {
    if (req.user.role === 'IQAC' || req.user.role === 'HR' || req.user.role === 'President') {
