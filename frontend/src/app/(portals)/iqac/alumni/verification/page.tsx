...
export default function IqacAlumniVerificationPage() {
  const api = useAuthedApi();
  const [queue, setQueue] = useState<Row[]>([]);

  const load = () =>
    void api.get<Row[]>('/iqac/alumni/verification-queue').then(setQueue).catch(() => setQueue([]));
  ...
  async function verify(alumniId: string, action: 'approve' | 'reject') {
    try {
      await api.patch(`/iqac/alumni/profiles/${alumniId}/verify`, { action });
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4 md:p-6">
      <IqacPageHeader
        ...
        description="Approve graduate registrations before they access the Falcon Alumni Network."
      />
      {queue.map((row) => (
