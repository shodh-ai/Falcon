...
import { ArrowLeft, CheckCircle2, CircleDollarSign, GitCompare, Pencil, ShieldCheck, Truck } from 'lucide-react';
import { useAuthedApi } from '@/lib/api';
import { createAcquisitionsApi, type AcquisitionDetail, type AcquisitionDraftCorrectionInput, type AcquisitionDraftInput, type AcquisitionFundingSource } from '@/lib/api/api.acquisitions';
import { toast } from '@/lib/notifications/falcon-toast';
import { AcquisitionStatus } from '@/components/acquisitions/AcquisitionStatus';
import { AcquisitionDraftEditor } from '@/components/acquisitions/AcquisitionDraftEditor';
...
export default function AcquisitionDetailPage({ params }: { params: Promise<{ versionId: string }> }) {
  ...
  }).catch(() => { setDetail(null); setLoadDenied(true); }), [api, versionId]);
  useEffect(() => { void load(); }, [load]);
  useEffect(() => {
    if (detail?.status !== 'DRAFT' || !detail.allowed_actions?.edit) return;
    void api.fundingSources().then(setFundingSources).catch((error) => toast.error(error.message));
  }, [api, detail?.allowed_actions?.edit, detail?.status]);
  if (loadDenied) return <div className="mx-auto max-w-lg p-8"><Card><CardHeader><CardTitle>Acquisition unavailable</CardTitle></CardHeader><CardContent className="space-y-4"><p className="text-sm text-muted-foreground">You do not have access to this acquisition, or it no longer exists.</p><Link href="/finance/acquisitions"><Button variant="outline"><ArrowLeft className="mr-2 h-4 w-4"/>Return to your acquisition queue</Button></Link></CardContent></Card></div>;
  if (!detail) return <div className="p-8 text-sm text-muted-foreground">Loading secure acquisition snapshot…</div>;
  const vendorSelectionLocked = detail.status !== 'VENDOR_REVIEW' && detail.lines.some((line) => Boolean(line.selected_vendor_id));
  ...
  const saveDraftCorrections = async (input: AcquisitionDraftCorrectionInput) => {
    setBusy(true);
    try {
      await api.correctDraft(versionId, input);
      await load();
      setEditing(false);
  ...
  return <div className="space-y-6 p-6">
    ...
    <Tabs defaultValue="lines">
      ...
      <TabsContent value="vendors" className="space-y-4">
        {vendorSelectionLocked && <Card className="border-slate-200 bg-slate-50"><CardContent className="flex flex-wrap items-center justify-between gap-3 p-4"><p className="text-sm text-muted-foreground">Vendor selection is locked because its budget reservation and DoFA route are pinned. Editing requires a new acquisition version.</p>{detail.allowed_actions?.withdraw && <Button variant="outline" disabled={busy} onClick={() => void act('withdraw')}>Withdraw to edit vendor selection</Button>}{detail.allowed_actions?.amend && <Button disabled={busy} onClick={() => void act('amend')}>Create amendment to edit</Button>}</CardContent></Card>}
        {detail.lines.map((line) => <Card key={line.line_id} className={vendorSelectionLocked ? 'bg-slate-50 opacity-75' : undefined}><CardHeader><CardTitle className="text-base">{line.product_name}</CardTitle></CardHeader><CardContent className="space-y-3">{byLine(line.line_id).map((candidate) => { const chosenVendor = selections[line.line_id]?.vendor_id ?? line.selected_vendor_id ?? ''; return <label key={candidate.recommendation_id} className={`grid gap-2 rounded-lg border p-3 md:grid-cols-[auto_1fr_auto] ${detail.status === 'VENDOR_REVIEW' ? 'cursor-pointer' : 'cursor-not-allowed'}`}><input type="radio" name={line.line_id} disabled={detail.status !== 'VENDOR_REVIEW'} checked={chosenVendor === candidate.vendor_id} onChange={() => setSelections({...selections,[line.line_id]:{vendor_id:candidate.vendor_id,deviation_justification:''}})}/><div><strong>#{candidate.rank} {candidate.vendor_name}{line.selected_vendor_id === candidate.vendor_id ? ' · Selected' : ''}</strong><p className="text-xs text-muted-foreground">{candidate.explanation}</p><p className="mt-1 text-xs">Confidence: {candidate.confidence} · policy v{candidate.scoring_policy_version}</p></div><strong>{Number(candidate.final_score).toFixed(2)}</strong></label>})}{!byLine(line.line_id).length && <p className="text-sm text-muted-foreground">Run the deterministic scoring policy to produce eligible candidates.</p>}{detail.status === 'VENDOR_REVIEW' && selections[line.line_id] && byLine(line.line_id)[0]?.vendor_id !== selections[line.line_id].vendor_id && <Input placeholder="Required deviation justification (minimum 20 characters)" value={selections[line.line_id].deviation_justification} onChange={(e) => setSelections({...selections,[line.line_id]:{...selections[line.line_id],deviation_justification:e.target.value}})}/>}</CardContent></Card>)}
        {detail.allowed_actions?.vendor_review && <Button disabled={busy} onClick={() => void submitVendors()}>Confirm vendors and reserve budget</Button>}
      </TabsContent>
