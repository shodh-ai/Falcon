...
export default function OnboardingStep1Page() {
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      await api.post('/api/student/onboarding/reset-password', {
        current_password: currentPassword,
        new_password: newPassword,
        confirm_password: confirmPassword,
      });
      await refreshUser();
      toast.success('Password updated. Continue to document upload.');
      router.replace('/student/onboarding/step-2');
    } catch (err) {
  ...
  return (
    <Card className="border-sgvu-navy/10 shadow-lg">
      <CardHeader>
        ...
        <CardDescription>
          For security, replace the default password before accessing the student portal.
        </CardDescription>
      </CardHeader>
      <CardContent>
