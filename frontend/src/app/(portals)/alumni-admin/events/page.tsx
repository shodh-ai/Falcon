...
export default function AlumniAdminEventsPage() {
  ...
  const load = () => void api.get<EventRow[]>('/api/alumni-admin/events').then(setEvents).catch(() => setEvents([]));
  ...
  async function create() {
    ...
    try {
      await api.post('/api/alumni-admin/events', { title, event_date: date, venue });
  ...
  return (
    <div className="mx-auto max-w-5xl space-y-4 p-4 md:p-6">
      <AlumniPageHeader title="Event Manager" description="Create events and track RSVP counts on the alumni portal." />
      <Card>
        <CardHeader>
