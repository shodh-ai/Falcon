...
export default function SuperAdminCourseMapperPage() {
  const { token } = useAuth();
  ...
  async function downloadTemplate() {
    if (!token) return;
    const res = await fetch(`${API_URL}/api/super-admin/academics/course-mapper/template`, {
      headers: authHeaders,
    });
    if (!res.ok) {
  ...
  const uploadForPreview = useCallback(
    async (file: File) => {
      ...
      try {
        ...
        const res = await fetch(`${API_URL}/api/super-admin/academics/course-mapper/preview`, {
          method: 'POST',
          headers: authHeaders,
          body: formData,
        });
    ...
    [token, authHeaders, academicYear],
  );
  ...
  async function confirmExecute() {
    ...
    setExecuting(true);
    try {
      const res = await fetch(`${API_URL}/api/super-admin/academics/course-mapper/execute`, {
        method: 'POST',
      ...
      setResult(data);
      setStep('done');
      toast.success('Course allocation matrix imported successfully');
    } catch (e) {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6 p-6">
      <div>
        <h1 className="text-2xl font-bold text-sgvu-navy">Smart Bulk Course Mapper</h1>
        <p className="mt-1 max-w-3xl text-sm text-muted-foreground">
          Upload the HOD&apos;s Course Allocation Matrix (Teaching Load) Excel file. Falcon will
          auto-create subjects, map faculty, provision LMS workspaces, and flag unassigned (NF) rows
          for HOD follow-up.
        </p>
      </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            >
              ...
              <p className="mt-1 text-sm text-muted-foreground">Teaching Load matrix from the HOD</p>
      ...
      {step === 'preview' && preview && (
        <>
          <Card>
            <CardHeader>
              <CardTitle className="text-base">Step 2 — Preview & conflict resolution</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
          ...
          <CardContent className="space-y-3 text-sm">
            ...
            {result.unassigned_count > 0 && (
              <p className="text-amber-800">
                {result.unassigned_count} row(s) saved as unassigned — HODs can assign faculty from
