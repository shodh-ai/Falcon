...
export function DepartmentFormDialog({
  ...
  useEffect(() => {
    if (!open) return;
    const legacyMatch =
      !department?.school_id && department?.description
        ? lookups.schools.find(
            (school) =>
              school.school_name.trim().toLowerCase() ===
              department.description?.trim().toLowerCase(),
          )
        : undefined;
    const nextCampusId = department?.campus_id
      ? String(department.campus_id)
      : legacyMatch?.campus_id
        ? String(legacyMatch.campus_id)
        : '';
    const nextSchoolId = department?.school_id
      ? String(department.school_id)
      : legacyMatch?.school_id
        ? String(legacyMatch.school_id)
        : '';
  ...
  }, [open, department, lookups.schools]);
