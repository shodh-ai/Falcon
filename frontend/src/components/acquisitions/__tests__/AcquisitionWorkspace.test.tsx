...
describe("AcquisitionWorkspace persona queue", () => {
  beforeEach(() => {
    ...
    mocks.get.mockImplementation((path: string) =>
      Promise.resolve(
        path.includes("funding-sources")
          ? [
              {
                funding_source_type: "PROJECT",
                funding_source_id: "fund-gvmc",
  ...
  it("opens a multi-line wizard with online/offline and funding controls", async () => {
    ...
    expect(
      screen.getByRole("combobox", { name: "Funding source" }),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getByRole("combobox", { name: "Product line 1 category" }),
    ).toHaveValue("ASSET");
    ...
    expect(screen.queryByPlaceholderText("Tax")).not.toBeInTheDocument();
    expect(screen.getByPlaceholderText("Price per unit")).toBeInTheDocument();
    expect(
      screen.getByPlaceholderText("Notes and General info (optional)"),
    ).toBeInTheDocument();
    expect(
    ...
    expect(screen.getByText("Upload completed template")).toBeInTheDocument();
  });
});
