...
function formatInr(amount: number | null | undefined) {
  if (amount == null) return 'No cap';
  return `₹${Number(amount).toLocaleString('en-IN')}`;
}

function CompiledMatrixPreview({
  bands,
  domain,
}: {
  bands: CompiledBand[];
  ...
  if (!bands.length) {
    return (
      <p className="text-xs text-muted-foreground">No approval bands on this graph yet.</p>
    );
  }
  ...
  return (
    <ul className="space-y-1.5">
      {bands.map((band, idx) => {
        const level = band.level_no ?? idx + 1;
        const title =
          band.label ?? band.rule_key ?? (domain === 'P2P' ? `Level ${level}` : `Band ${level + 1}`);
        const amount =
          domain === 'P2P'
            ? formatInr(band.max_amount_inr ?? band.amount_max)
            : band.amount_min != null || band.amount_max != null
              ? `${formatInr(band.amount_min ?? 0)} – ${formatInr(band.amount_max)}`
              : 'No amount cap';
        const sigs = band.required_roles?.join(' + ') || '—';

        return (
          <li
            key={`${title}-${level}-${idx}`}
            className="rounded-md border border-slate-200 bg-slate-50 px-2.5 py-2 text-xs"
          >
            <div className="font-semibold text-sgvu-navy">
              L{level} · {title}
            </div>
            <div className="mt-0.5 text-muted-foreground">{amount}</div>
            <div className="mt-0.5 text-muted-foreground">
              Signatures: {sigs}
              {band.required_signatures != null && band.required_signatures > 1
                ? ` (${band.required_signatures} required)`
                : ''}
            </div>
          </li>
        );
      ...
      {tab === 'audit' ? (
        ...
        <div className="flex flex-1 min-h-0 gap-3">
          ...
          <aside className="w-80 shrink-0 space-y-3 overflow-auto border rounded-md bg-white p-3 text-sm">
            ...
            {canUnlock && !canPropose && (
              <p className="rounded-md border border-amber-200 bg-amber-50 px-2.5 py-2 text-xs text-amber-950">
                <strong>CFO unlock-only.</strong> You may review graphs and unlock{' '}
                <code className="text-[10px]">PENDING_CFO</code> changes with OTP. Drafting and
                editing bands is restricted to IT (Campus Admin).
              </p>
            )}

            {canPropose && !canUnlock && selected?.status === 'PUBLISHED' && (
              <p className="rounded-md border border-slate-200 bg-slate-50 px-2.5 py-2 text-xs text-muted-foreground">
                Published graphs are read-only. Use <strong>New draft from canvas</strong> to propose
                a change — live matrices stay untouched until the CFO unlocks.
              </p>
            )}
            ...
            <div className="pt-2 border-t space-y-2">
              <div className="text-xs font-semibold text-sgvu-navy">Compiled matrix preview</div>
              <CompiledMatrixPreview bands={compiledFromNodes} domain={domain} />
            </div>
          </aside>
        </div>
      )}
